...
class StorePlaceRequest extends FormRequest
{
    ...
    public function build()
    {
        ...
        $placeData = $this->validated();

        $placeData['slug'] = $placeData['name'];

        $place = Place::create($placeData);

        $this->placeImages($place, $this->images);

        $place->tags()->attach($this->tag_ids);

        foreach ($this->specification_ids as $specification) {

            $place->specifications()->attach($specification['id'], ['value' => $specification['value']]);
        }
